...
import com.product.api.dto.DtoUpdateProductCategory;
...
public class CtrlProduct {
	...
	public ResponseEntity<List<DtoProductList>> listProducts(@PathVariable("category_id") Integer category_id){
		List<DtoProductList> productList = svc.listProducts(category_id);
		return new ResponseEntity<>(productList, HttpStatus.OK);
	} 

	@PutMapping("/{gtin}/category")
	public ResponseEntity<ApiResponse> updateProductCategory(@PathVariable("gtin") String gtin, @Valid @RequestBody DtoUpdateProductCategory in, BindingResult bindingResult){
		if(bindingResult.hasErrors())
			throw new ApiException(HttpStatus.BAD_REQUEST, bindingResult.getAllErrors().get(0).getDefaultMessage());
		Integer category_id = in.getCategory_id();
